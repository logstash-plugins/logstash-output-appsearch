...
| <<plugins-{type}s-{plugin}-service_type>> |<<string,string>>|No
|=======================================================================
...
[id="plugins-{type}s-{plugin}-service_type"]
===== `service_type`

  * Value type is <<string,string>>
  * Default value is `saas`

The service type of the App Search API. Valid values are `self-managed` for
https://www.elastic.co/downloads/app-search[Self-Managed Elastic App Search] or `saas` 
for the https://www.elastic.co/cloud/app-search-service[Elastic App Search managed service].
